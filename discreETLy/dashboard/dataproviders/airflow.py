...
class AirflowDBDataProvider:
    ...
    def get_history(self, days):
      ...
      SELECT dag_id, date, state FROM (
            ...
            FROM (
                SELECT *,
                    CASE
                        WHEN split_part(reverse(dag_id), 'v_', 1) like '[0-9]+\.[0-9]'
                        THEN substring(dag_id, 1, length(dag_id) - 1 - position('v_' in reverse(dag_id)))
                        ELSE dag_id
                    END as clean_dag_id from dag_run) dag_run, (SELECT ''::text AS dag_id_current, 0::int AS row_number) as t
    ...
    def get_last_successful_tasks(self):
        ...
        for row in data:
            row_dict = {}
            row_dict.update(row)
            row_dict['dag_name'] = clean_dag_id(row['dag_id'])
            key = row_dict['dag_name'] + row_dict['task_id']

            # row['dag_name'] = clean_dag_id(row['dag_id'])
            # key = row['dag_name'] + row['task_id']
            if key in result and result[key].end_date > row_dict['end_date']:
    ...
    def get_newest_task_instances(self):
        ...
        JOIN dag ON dag.dag_id = dr.dag_id AND is_active = TRUE AND is_paused = FALSE '''.replace("\n", "")
        ...
        for row in data:
            ...
            row_dict['dag_name'] = clean_dag_id(row['dag_id'])
            key = row_dict['dag_name'] + row_dict['task_id']
            if key in result and row_dict['end_date'] and result[key].end_date > row_dict['end_date']:
                continue  # duplicate with dag old version

            if row_dict['dag_name'] in self.config.get('TECHNICAL_ETLS', set()):
    ...
    def get_dags_status(self):
        ...
        JOIN dag ON dag.dag_id = dag_run.dag_id AND is_active = TRUE AND is_paused = FALSE '''.replace("\n", "")
        return [{
